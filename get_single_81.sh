...
sizes_1=()
max1=0
for filename in sim\ 81\ 1\ *.csv; do
	arq_1+=("$filename")
...
sizes_095=()
max095=0
for filename in sim\ 81\ 0.95\ *.csv; do
        arq_095+=("$filename")
        cur=$(cat "$filename" | wc -l)
        sizes_095+=("$cur")
	if (( $cur > $max095 )); then
...
cp "${arq_1[$selected_1]}" "sim 081 1 selected.csv"
cp "${arq_neg1[$selected_neg1]}" "sim 081 -1 selected.csv"
cp "${arq_095[$selected_095]}" "sim 081 0.95 selected.csv"
